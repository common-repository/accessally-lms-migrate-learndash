/* global accessally_learndash_convert_object */
...
jQuery(document).ready(function ($) {
	...
	var $wait_overlay = $('#accessally-learndash-convert-wait-overlay');
	$(document).on('click touchend', '[accessally-convert-course]', function() {
		var conf = confirm('Do you want to convert all items in this LearnDash course to regular WordPress pages?')
		if(conf !== true){
			return false;
		}
		$wait_overlay.show();
		var course_id = $(this).attr('accessally-convert-course'),
			conversion_operation = $('#accessally-learndash-operation-' + course_id).val(),
			data = {
				action: 'accessally_learndash_convert',
				id: course_id,
				op: conversion_operation,
				nonce: accessally_learndash_convert_object.nonce
			};

		$.ajax({
			type: "POST",
			url: accessally_learndash_convert_object.ajax_url,
			data: data,
	...
	function process_convert_result(response) {
		...
		if ('status' in result) {
			...
			var $display_elem = $('#accessally-learndash-convert-container');
	...
	$(document).on('click touchend', '[accessally-revert-course]', function() {
		var conf = confirm('Do you want to change previous converted pages back to a LearnDash Course?'),
		...
		$wait_overlay.show();
		var data = {
				action: 'accessally_learndash_revert',
				id: course_id,
				nonce: accessally_learndash_convert_object.nonce
			};
